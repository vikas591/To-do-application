document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkReminders() {
        ...
        tasks.forEach(task => {
            if (!task.completed && task.dueDate) {
                ...
                // Widen Window: Trigger if within 5 mins OR if we missed it (up to 5 mins overdue)
                // This 'Catch-up' logic helps if phone was sleeping
                if (minutesDiff <= 5 && minutesDiff > -5) {
                    if (!task.reminded) {
                        try {
                            if ('Notification' in window && Notification.permission === 'granted') {
                                new Notification('Task Due Soon! ⏰', {
                                    body: `"${task.text}" is due now/soon!`,
                                    icon: 'https://cdn-icons-png.flaticon.com/512/762/762696.png',
                                    requireInteraction: true // Keep on screen until clicked
                                });
                                // Safe Audio Playback
                                try { playSound('notify'); } catch (e) { console.log('Audio blocked', e); }
                            }
                        } catch (err) {
    ...
    function addTask() {
        ...
        // --- Safe Permission Request Capture ---
